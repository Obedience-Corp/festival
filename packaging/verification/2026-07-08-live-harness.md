...
- `just plugin dist-check`: initially FAIL before the fix in this verification pass. The dry-run checked for `.codex-plugin/plugin.json`, but the generated Codex bundle and target check use `.codex-plugin/.codex-plugin/plugin.json`. After updating the dry-run expected surface, PASS: all four distribution surfaces were present and no push was performed.
...
Result: PASS for local Codex marketplace parsing/install. This proves Codex CLI `0.142.5` accepts the generated `.agents/plugins/marketplace.json`, the generated Codex manifest at `.codex-plugin/.codex-plugin/plugin.json`, the generated `_generated` JSON keys, and the manifest's `interface` block containing `category: "project-management"` and `capabilities: ["skills", "hooks"]`.
