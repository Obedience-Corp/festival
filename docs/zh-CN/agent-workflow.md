...
下一次会话，不管你使用 Claude Code、Codex 还是 Cursor，都从同一个命令开始：
...
mark completed work with `fest task completed`, and commit with `fest commit`.
If context is missing, inspect the festival files before making assumptions.
```
