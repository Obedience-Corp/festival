...
instruction does is point the agent at the right working directory. Where that is
depends on the work: **planning** a festival runs the loop in the festival directory
itself, while **implementing** one runs it in the festival's linked project or
worktree. `fest go` (the `fgo` shortcut) toggles between a festival and its linked
working directory, so you move between the two as the work moves from planning to
implementation. The bracketed paths below are where you point the agent (in most
agent harnesses, with path autocomplete).

Plan a festival:

> Navigate to `<festival path>` and run the fest next loop: run `fest next`, do the
> step it prints, advance, and repeat. Stop at any approval gate and ask me.

Implement a festival, when you know where it is linked:
...
Implement a festival, when you are not sure where it is linked:

> Navigate to `<festival path>`, run `fest go` to toggle to its linked working
> directory, then run the fest next loop there.

Links are created with `fest link` from inside a festival and listed with
`fest links`; if a festival has no link yet, `fest link` attaches one.
...
a festival. It organizes work as phases, sequences, and tasks. Plan it from the
festival directory, then implement it from the festival's linked project or worktree
(`fest go` toggles between the two). `fest next` walks that structure by dependency
and progress; `fest commit` records each step with a traceability tag and advances
the loop:
