...
- Result: PASS. The second run is byte-identical to the first across `plugins/festival/`,
...
- Present: `plugins/festival/.codex-plugin/plugin.json`, `.cursor-plugin/plugin.json`, `.opencode/plugins/festival.js`,
...
- Command: `just plugin bump 1.1.2`; read the version from plugin.json, marketplace.json,
  `plugins/festival/.codex-plugin/plugin.json`, `.cursor-plugin/plugin.json`, `gemini-extension.json`, and
...
- Codex: `SessionStart` command hook in `plugins/festival/hooks/hooks.json` (`${PLUGIN_ROOT}`).
...
- Present: `packaging/README.md` (multi-target layout, source-of-truth rule, generate/drift/version,
  distribution), `packaging/DISTRIBUTION.md`, root `AGENTS.md`, `plugins/festival/README.md`,
